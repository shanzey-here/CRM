...
export default function MobileRunSheetPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!jobDetails && !error) {
    return <div className="p-8 text-center text-slate-500">Loading run sheet...</div>
  }
  ...
  return (
    <div className="p-4 sm:p-8 space-y-4 max-w-4xl mx-auto pb-24">
      <Link href="/crew" className="inline-flex items-center text-blue-600 hover:text-blue-700 font-medium">
        <ArrowLeft className="w-4 h-4 mr-1" /> Back to Jobs
      </Link>

      {/* Sync Status Toolbar */}
      <div className="flex flex-col space-y-2 bg-white p-4 rounded-lg shadow-sm border border-slate-200">
        ...
        {hasPending && (
          <div className="pt-2 mt-2 border-t border-slate-200 flex items-center justify-between text-sm">
      ...
      {jobDetails && (
        <div className="bg-white p-4 sm:p-8 rounded-lg shadow-sm border border-slate-200 overflow-hidden">
          <JobSheetContent jobDetails={jobDetails} />

          <div className="mt-8 pt-8 border-t border-slate-200">
            <h2 className="text-xl font-bold uppercase text-slate-500 tracking-wider mb-4">Job Photos</h2>

            {/* Gallery */}
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
              {uploadedPhotos.map((photo: any) => (
                <div key={photo.id} className="bg-slate-100 aspect-square rounded-lg flex items-center justify-center p-2 text-center text-xs text-slate-500 overflow-hidden relative border border-slate-200">
              ...
              {pendingUploads.map(pending => (
                <div key={pending.id} className={`bg-amber-50 aspect-square rounded-lg flex flex-col items-center justify-center p-2 text-center text-xs border-2 ${pending.status === 'failed' ? 'border-red-400' : 'border-amber-400'} relative overflow-hidden`}>
                  ...
                  <div className="z-10 bg-white/90 p-1 rounded font-medium">
                    {pending.status === 'failed' ? <span className="text-red-600">Failed</span> : <span className="text-amber-700">Pending</span>}
                    {pending.caption && <div className="truncate w-20 text-[10px] text-slate-600 mt-1">{pending.caption}</div>}
                  </div>
                </div>
              ))}
