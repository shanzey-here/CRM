...
export function CreateInventoryForm({ onClose }: { onClose: () => void }) {
  ...
  const onSubmit = (data: z.input<typeof insertInventoryItemSchema>) => {
    startTransition(async () => {
      try {
        ...
        await createInventoryAction(formData)
        onClose()
      } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      <div className="flex min-h-screen items-end justify-center px-4 pt-4 pb-20 text-center sm:items-center sm:p-0">
        ...
        <div className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6 w-full">
          <div>
            <h3 className="text-lg font-medium leading-6 text-slate-900" id="modal-title">Add Inventory Item</h3>
            <form onSubmit={handleSubmit(onSubmit)} className="mt-5 space-y-4">
              ...
              <div>
                <label htmlFor="default_volume" className="block text-sm font-medium text-slate-700">Default Volume (cubic feet)</label>
                <div className="mt-1">
                  <input
                    type="number"
                    step="0.01"
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3 text-sm">
                  ...
                  <p className="text-slate-500">Item will be available for quoting.</p>
                </div>
              </div>
