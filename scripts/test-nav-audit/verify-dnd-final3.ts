...
async function main() {
  ...
  page.on('response', async (res) => {
    if (res.request().method() === 'POST' && res.url().endsWith('/office/leads')) {
      ...
      netLog.push(`status=${res.status()} bodyStart=${body.slice(0, 200)}`)
    }
  })
  ...
  // Column 2 (Survey Scheduled, idx 1) -> Column 3 (Quote Sent, idx 2). Both
  // fully on-screen at this viewport width, no scrolling needed.
  ...
  console.log('gripBox:', JSON.stringify(gripBox), 'dstBox:', JSON.stringify(dstBox))
  ...
  await page.screenshot({ path: 'D:/CRM/scripts/test-nav-audit/final3-mid-drag.png' })
  ...
  await page.screenshot({ path: 'D:/CRM/scripts/test-nav-audit/final3-after-drop.png' })
