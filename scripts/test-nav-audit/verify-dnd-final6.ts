...
async function main() {
  ...
  const netLog: string[] = []
  page.on('request', (req) => {
    if (req.method() === 'POST' && req.url().endsWith('/office/leads')) {
      netLog.push(`>> REQUEST at ${Date.now()} headers.next-action=${req.headers()['next-action']}`)
    }
  })
  page.on('response', async (res) => {
    if (res.request().method() === 'POST' && res.url().endsWith('/office/leads')) {
      const body = await res.text().catch(() => '(failed to read body)')
      netLog.push(`<< RESPONSE at ${Date.now()} status=${res.status()} body=${body.slice(0, 500)}`)
    }
  })
  ...
  const dstDropZone = columns.nth(0).locator('div.flex.flex-col.gap-2').first()
  const card = srcDropZone.locator('> div').first()
  const grip = card.locator('[aria-label="Drag to reorder"]')
  const cardText = await card.evaluate((el: HTMLElement) => el.textContent?.slice(0, 36))
  console.log('Dragging card:', cardText)
  ...
  const highlight = await columns.nth(0).evaluate((el: HTMLElement) => el.style.boxShadow)
  console.log('isOver boxShadow at drop time:', JSON.stringify(highlight))
  await page.screenshot({ path: 'D:/CRM/scripts/test-nav-audit/final-mid-drag.png' })

  await page.mouse.up()

  // Give the server action generous time in this environment (observed
  // 5-30s+ first-hit latencies all session) rather than a short fixed wait.
  await page.waitForTimeout(15000)

  console.log('Network:')
  netLog.forEach((l) => console.log('  ' + l))
  if (netLog.length === 0) console.log('  (none captured — action never called)')

  console.log('After (same session, 15s after drop):', JSON.stringify(await getCounts(page)))
  const errorBanner = await page.locator('[role="alert"]').textContent().catch(() => null)
  console.log('Error banner text:', errorBanner)
  await page.screenshot({ path: 'D:/CRM/scripts/test-nav-audit/final-after-drop.png' })
