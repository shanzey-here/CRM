...
async function main() {
  ...
  page.on('response', async (res) => {
    if (res.request().method() === 'POST' && res.url().endsWith('/office/leads')) {
      ...
      netLog.push(`status=${res.status()} bodyStart=${body.slice(0, 250)}`)
    }
  })
  ...
  // Extended warm-up: visit a lead detail page and come back, and let the
  // notification poll cycle at least once, before attempting the timed drag —
  // gives the dev server's slow cold-compile a chance to settle first.
  console.log('Warming up: visiting a lead detail page...')
  const firstCard = page.locator('.overflow-x-auto p.font-semibold').first()
  await firstCard.click()
  await page.waitForURL((u) => u.pathname !== '/office/leads', { timeout: 60000 }).catch(() => console.log('  (detail nav did not complete in time)'))
  await page.waitForTimeout(2000)
  await page.goBack({ waitUntil: 'domcontentloaded' })
  await page.waitForTimeout(3000)
  console.log('Warm-up complete. URL now:', page.url())
  ...
  const dstDropZone = columns.nth(2).locator('div.flex.flex-col.gap-2').first()
  ...
  const highlight = await columns.nth(2).evaluate((el: HTMLElement) => el.style.boxShadow)
