...
async function checkAs(email: string, password: string) {
  ...
  console.log(`${email} -> final URL after requesting /office/storage:`, page.url())
  const bodyText = await page.locator('body').innerText()
  console.log(`${email} -> page shows crates heading?`, bodyText.includes('Crates') && bodyText.includes('New Crate'))
  await browser.close()
}
