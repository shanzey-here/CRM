...
async function main() {
  ...
  console.log('=== Real Settings page render (window temporarily = 10, tenant has 10 real unedited resolutions) ===')
  ...
  console.log('auto_send input still disabled?', /value="auto_send"[^>]*disabled/.test(html))

  console.log('\n=== Real direct server-side attempt to set auto_send (should now succeed) ===')
  ...
  console.log('\nActual DB mode after attempt:', settings?.ai_quoting_mode)
}
main()
