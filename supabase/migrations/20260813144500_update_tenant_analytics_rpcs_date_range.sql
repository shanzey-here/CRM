-- Migration: Update tenant analytics RPCs to use explicit start/end dates instead of months,
-- allowing for custom date range selection. Buckets dynamically by Day if range <= 60 days, else Month.

DROP FUNCTION IF EXISTS get_tenant_quotes_bookings_over_time(uuid, int);
DROP FUNCTION IF EXISTS get_tenant_revenue_over_time(uuid, int);
DROP FUNCTION IF EXISTS get_tenant_new_clients_over_time(uuid, int);
...
BEGIN
  ...
  FROM periods p
  LEFT JOIN clients c ON p.b_date = c.b_date
  ORDER BY p.period_start;
END;
$$;
